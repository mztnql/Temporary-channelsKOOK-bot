...
async def guild_user(guild_id, user_id, text):
    channel_id_1 = await bot.client.gate.request('GET', 'channel-user/get-joined-channel', params={'guild_id': guild_id, 'user_id': user_id})
    # 储存频道id字典
    channel_id = []
    channel_id2 = []
    # 将接口返回值里的频道id储存进列表
...
async def create(guild_id, a, target_id, user_id):
    ...
    if guild_id not in user_date:
        # 创建频道分组并且获得分组id
        grouping = await create_category2(guild_id, '临时频道')
        # 将频道分组id写入字典，后续再写入文件
...
async def print_btn_value1(b: Bot, e: Event):
    ...
    # 一键删除所有临时频道
        ...
        if guild_id in user_date:
            ...
            for channel_id in user_date[guild_id]['频道id']:
                ...
                await bot.client.delete_channel(channel_id)
            # 直接覆写
            user_date[guild_id] = {'频道分组id': channel_grouping, '频道id': {}}
            # 将字典存入文件
            await write_file(path_id, user_date)
    ...
    # 一键踢出机器人
        ...
        # 更新卡片，防止踢出机器人后还有人使用
        await upd_card(e.body['msg_id'], CardMessage(Card(Module.Header('机器人已从当前服务器踢出'), Module.Section('如果失败请联系开发者'))))
        # 判断机器人有没有存这个服务器的数据
        if guild_id in user_date:
            # 顺带删除所创建的频道及分组
            # 获取分组id
